...
```{r}
croix_tidy <- croix %>% unnest_tokens(word,fulltext_org)
...
```
Not surprisingly, particles are the most common words we find. This is not particularly interesting for us in this inquiry, as we want to see which words are specific to the individual month. The particles will appear in all month. The first step is finding a measurement that will allow us to compare the frequency of words across the months. We can do this by calculating the word’s, or the term’s, frequency: 

$$frequency=\frac{n_{term}}{N_{month}}$$
...
```{r}
croix_counts <- croix_tidy %>% 
  count(m,word, sort = TRUE) %>% 
  left_join(totalwords, by = "m")
croix_counts %>% 
  arrange(m)
```
...
$$\text{frequency of "the" in August}=\frac{3934}{64822}=0.06068927$$
...
```
Many people who have dipped their toes in the text mining/data mining sea will have seen wordclouds showing the most used words in a text. In this visualisation we are going to create a wordcloud for each month showing the words with the highest tf_idf from that month. By doing so we will get a nice overview of what words are specific and important to each month. Remember that words which appear a lot across the months will not show here. 
```{r}
croix_tfidf %>% 
...
```
Let's see what is going on with "rioters" in October. Be using our initial dataframe we can specify that we are only interested in October and there after that we only want to see the articles that have the word "rioters" or "2.00" in them.

```{r}
croix %>% 
  filter(m == 11) %>% 
  filter(str_detect(fulltext_org, "rioters|2.00")) %>% 
...
Note: Adela updated slightly in Dec 2022
